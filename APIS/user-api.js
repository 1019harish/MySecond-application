...
userApi.post("/login", expressErrorHandler( async(req,res,next)=>{
    ...
    console.log("creds is : " , credentials)
    ...
    if(user===null){
        ...
        if(status===false){
            ...
            let token= await jwt.sign({username:credentials.username},process.env.SECRETKEY,{expiresIn: 10 })
            //send token to client
            res.send({message:"Login success",token:token,username:credentials.username})
            console.log("token is "  ,token);
            console.log("usernae is :" , credentials.username)
        }
    }
}))
...
userApi.post("/addtocart",expressErrorHandler(async(req,res,next)=>{
    let userCartCollectionObj=req.app.get("userCollectionObj")
